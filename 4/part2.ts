...
const input = inputFull
  ...
  .map(line => ({
    id: Number(line.id),
    instances: 1,
    winnings: line.numbers
      .reduce((wins, ownNumber) => line.winning.includes(ownNumber) ? wins+1: wins,0)
  }))

const map = input
  .reduce((map, card) => map.set(card.id, card) , new Map<number, (typeof input)[number]>())
...
console.log(map, [...map.values()].reduce((score, card) => score + card.instances, 0));
