...
function isInSet(pt: Pt, set: Pt[]){
    return set.some(p2 => pt[0] === p2[0] && p2[1] === pt[1])
}
...
const gottem = new Array<Pt>()

function castHorizon(p: Pt, dir: -1|1, line: Pt[]){
    ...
    while(pt[0] >= bounds.xmin && pt[0] <= bounds.xmax){
        if(vert.includes(cell(pt)!) && isInSet(pt, line)){
            intersects.push([...pt])
        }
